...
import { View, Text, TextInput, TouchableOpacity, SafeAreaView, ActivityIndicator, StyleSheet, ScrollView, Switch } from 'react-native';
...
const EditUserScreen = ({ route, navigation }) => {
  ...
  const [classId, setClassId] = useState(user.classId || null);
  const [isHosteller, setIsHosteller] = useState(user.isHosteller || false);

  const [adminPermissionLevel, setAdminPermissionLevel] = useState(user.permissionLevel || null);
  ...
  const handleUpdateUser = async () => {
    ...
    if (role === 'admin') {
        if (!adminPermissionLevel) {
            Toast.show({ type: 'error', text2: 'Please select an admin permission level.' });
            return;
        }
        ...
        switch (adminPermissionLevel) {
            ...
            case 'hod':
                if (!department) {
                    Toast.show({ type: 'error', text1: 'Failed', text2: 'Please select a department for the HOD.' });
                    return;
                }
    ...
    <SafeAreaView className="flex-1 bg-gray-100">
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 100 }}>
        ...
        <RNPickerSelect
          ...
          items={userProfile?.permissionLevel === 'superadmin' ? roleItems : roleItems.filter(r => r.value !== 'admin')}
          style={pickerSelectStyles}
          value={role}
          Icon={() => <Icon name="chevron-down" size={24} color="gray" />}
        />
        ...
        {role === 'student' && (
          <>
            ...
            />
            <Text className="text-base text-gray-600 mb-2">Assign to Class</Text>
            <RNPickerSelect
        ...
        {role === 'admin' && (
          <>
            ...
            <TextInput
              ...
              autoCapitalize="none"
            />
            <Text className="text-base text-gray-600 mb-2">Admin Permission Level</Text>
            <RNPickerSelect
