...
const bulkCreateUsers = async (req, res) => {
  ...
  try {
    ...
    for (const [index, row] of results.entries()) {
      ...
      try {
        if (
          adminUser.permissionLevel === "hod" &&
          department !== adminUser.adminDomain
        ) {
          continue;
        }
        ...
        if (!displayName || !email || !password || !role) {
          throw new Error(
            "Missing required fields: displayName, email, password, role."
          );
        }
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (role === "student") {
      if (academicInfo) {
        updateData["academicInfo.urn"] = academicInfo.urn;
      }
      if (batch) {
      ...
      if (classId !== undefined) {
        updateData["academicInfo.classId"] = classId;
      }
